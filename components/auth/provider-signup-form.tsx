...
import { signUpProviderAction } from '@/app/actions/auth-actions';
import { PhoneInput } from '@/components/shared/phone-input';
import { AlertCircle, Loader2, ArrowRight, Zap, ClipboardList, Users, Headphones, CheckCircle2, X } from 'lucide-react';

const PROVIDER_BENEFITS = [
    { icon: Zap,           text: 'Proposez vos services à la demande' },
    { icon: ClipboardList, text: 'Gérez vos commandes en temps réel' },
    { icon: Users,         text: 'Visibilité auprès des voyageurs' },
    { icon: Headphones,    text: 'Accompagnement de notre équipe' },
];

export function ProviderSignupForm({ initialError }: { initialError?: string }) {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        startTransition(async () => {
            try {
                await signUpProviderAction(formData);
            } catch (err: any) {
    ...
    return (
        <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-amber-100 p-8 sm:p-10 relative overflow-hidden transition-all duration-300">
            {/* Décorations de fond */}
            <div className="absolute top-0 left-0 w-56 h-56 bg-amber-50 rounded-full blur-3xl -z-10 transform -translate-x-24 -translate-y-24 pointer-events-none" />
            <div className="absolute bottom-0 right-0 w-40 h-40 bg-orange-50 rounded-full blur-2xl -z-10 transform translate-x-14 translate-y-14 pointer-events-none" />
            ...
            <p className="text-gray-500 text-sm mb-5">
                Proposez vos services aux voyageurs qui séjournent à Kribi.
            </p>

            {/* Bénéfices */}
            <div className="mb-7 rounded-2xl bg-amber-50/80 border border-amber-100 p-4">
                <ul className="grid grid-cols-2 gap-2.5">
                    {PROVIDER_BENEFITS.map(({ icon: Icon, text }) => (
            ...
            {error && (
                <div className="mb-6 rounded-2xl bg-red-50 border border-red-200 p-4 flex items-start gap-3 text-red-900 text-sm shadow-md animate-in fade-in slide-in-from-top-2 duration-300">
                    ...
                    <div className="flex-1">
                        <p className="font-bold text-red-900">Erreur d'inscription prestataire</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="Marie-Claire Bella"
                    />
                </div>
                ...
                <div>
                    ...
                    <input
                        ...
                        disabled={isPending || !!successMessage}
                        className="w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-amber-400 focus:border-transparent transition disabled:opacity-60"
                        placeholder="Kribi Express Transfer"
                    />
                </div>
                ...
                <button
                    type="submit"
                    disabled={isPending || !!successMessage}
                    className="w-full font-bold rounded-full py-3.5 bg-amber-500 hover:bg-amber-600 text-white shadow-lg shadow-amber-500/30 transition duration-200 flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed active:scale-[0.99] mt-2"
                >
                ...
                <p className="text-xs text-gray-400 text-center pt-1">
                    ✅ Votre compte sera actif dès son inscription. Chaque service est ensuite modéré séparément.
                </p>
            </form>
            ...
            <div className="mt-6 pt-5 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-center gap-3 text-sm text-gray-500 text-center">
                <span>Déjà prestataire ?</span>
                <Link href="/connexion" className="font-bold text-amber-600 hover:underline">
                    Se connecter à votre espace →
                </Link>
                <span className="hidden sm:inline text-gray-300">|</span>
                <Link href="/devenir-hote" className="text-gray-400 hover:text-gray-600 hover:underline">
                    Vous êtes hôte ?
                </Link>
            </div>
        </div>
